...
### B. Tenant-Aware Dynamic Schema (`sys_TenantSchemaFields`)
- **Metadata Management:** Tenant field mappings are registered in `dbo.sys_TenantSchemaFields`. `FieldSourceType` selects the storage strategy: `1 = StandardColumn` (a physical column, exposed through the native dab-config mappings the admin worker generates), `2 = JsonAttribute` (a JSON scalar inside a system-managed container column, e.g. `CustomAttributesJson`), `3 = ForeignLookup` (reserved — loaded but not projected yet).
- **Tenant Binding:** Every tenant is its own DAB data source, declaring its tenant via the `tenant-id` data source option in dab-config.json. Metadata providers are created per data source, so each tenant's `SourceDefinition` is implicitly tenant-scoped and **no `HttpContext` claim inspection or per-request tenant lookup is required**.
- **In-Memory Caching:** Registry metadata is loaded into a thread-safe `ConcurrentDictionary` keyed by `(TenantId, EntityName)` via `ITenantSchemaRegistryService`, hydrated lazily before schema inference and re-hydrated by the hot reload engine. Reloads build a complete replacement snapshot and publish it with `Interlocked.Exchange`, so in-flight requests never observe a half-applied change. A failed read keeps the last-known-good snapshot. **Never issue a database roundtrip per API request to fetch schema mapping.**
- **Virtual Columns:** `MsSqlMetadataProvider` injects each JSON attribute into `SourceDefinition.Columns` as a `VirtualColumnDefinition`. Because DAB's GraphQL schema, REST field validation, default projection, OData filter model and wildcard field authorization all enumerate that dictionary, a dynamic attribute becomes a first-class field with no changes to any generic class. dab-config `mappings`/`fields` can only rename a field — they cannot create one.
- **Query Interception:** `MsSqlQueryBuilder` overrides `Build(Column)` — the single chokepoint for projections, filters, `ORDER BY`, `GROUP BY` and `HAVING` — to emit `CAST(JSON_VALUE([container], '$.path') AS type)`. Writes collapse every attribute of one container column into a single nested `JSON_MODIFY` assignment; attributes absent from a request are never touched, an explicit null removes its key, and an empty document is stored as SQL `NULL` on the container column.
